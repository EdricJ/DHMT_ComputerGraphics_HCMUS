...
namespace lab2
{
    public partial class Form1 : Form
    {
        ...
        //draw
        List<Draw> listDraw = new List<Draw>();     //chứa danh sách vẽ theo yêu cầu 
        ...
        private void form1_openGLControl_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left)
            {
                ...
                else if (rd_bt_Color.Checked) // Dang to mau
                {
                    //khởi tạo các biến mới để hỗ trợ tô màu 
                    FillColor fc = new FillColor(gl, e.Location, colorDialog1.Color);
                    FillColor_Scanline fcs = new FillColor_Scanline(gl, selectedPoints);
                    bool filled = false;
                    bool fill_sl = false;

                    if (rd_bt_Boundary.Checked)
                    {
                        started = true;     //tính thời gian tô 
                        ended = false;
                        fc.RE_BoundaryFill(Color.Black);
                        filled = true;
                    }
                    if (rd_bt_Flood.Checked)
                    {
                        started = true;
                        ended = false;
                        fc.RE_FloodFill(Color.White);
                        filled = true;
                    }
                    if (rd_bt_Scanline.Checked)
                    {
                        started = true;
                        ended = false;
                        fcs.fill();
                    ...
                    if (fill_sl)
                    {
                        listDraw.Add(new Draw(gl, OpenGL.GL_LINES, fcs.getFilledPoints(), colorDialog1.Color));
                    }

                    if (filled)
                    {
                        listDraw.Add(new Draw(gl, OpenGL.GL_POINTS, fc.getFilledPoints(), colorDialog1.Color));
                    }
                }
            }
        ...
        private void form1_openGLControl_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            else if (rd_bt_Color.Checked)
            {
                if (rd_bt_Boundary.Checked)
                {
                    started = false;    //khi đã tô xong 
                    ended = true;
                    // Bat dau stop watch dem thoi gian thuc thi ve hinh
                    time_case = System.Diagnostics.Stopwatch.StartNew();
                    resetPoints();
                }
                else if (rd_bt_Flood.Checked)
                {
                    started = false;    //khi đã tô xong 
                    ended = true;
                    // Bat dau stop watch dem thoi gian thuc thi ve hinh
                ...
                else if (rd_bt_Scanline.Checked)
                {
                    started = false;    //khi đã tô xong 
                    ended = true;
                    // Bat dau stop watch dem thoi gian thuc thi ve hinh
            ...
            else if (rd_bt_Select.Checked) // select and (dragging / scale / rotate / nothing)
            {
                ...
                else // select and scale
                {
                    ...
                    if (selectedType == SELECTED_POLYGON)  //select a polygon
                    {
                        ...
                        //begin transform
                        if (selectedControlPointIndex != -1) //scale neu select polygon là select control point => scale (co giãn)
                        {
                            //pStart la 1 diem control point, chọn vào các điểm control trên đường vẽ
                            //do scale (get updated control points after rotate)
                            // if one of them failed => do nothing =)))
                            if (af.scale(af.getControlPoints()[selectedControlPointIndex], pEnd))
                            {
                    ...
                    if (selectedType == SELECTED_ELLIPSE) //select a ellipse
                    {
                        ...
                        bool didTransform = false;
                        //begin transform
                        if (selectedControlPointIndex != -1) // neu select ellipse là select control point => scale
                        {
                            //scale, pStart la 1 diem control point
                            if (af.scale(af.getControlPoints()[selectedControlPointIndex], pEnd))
                            {
                                controlPoints = af.getControlPoints();
                                didTransform = true;
        ...
        private void form1_openGLControl_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            {
                if (rd_bt_Boundary.Checked)
                    started = true;         // bắt đầu tính thời gian tô màu, có thể thay bằng mouse click, vì không giữ và rê chuột để tô màu 
                else if (rd_bt_Flood.Checked)
                    started = true;
                else if(rd_bt_Scanline.Checked)
                    started = true;
