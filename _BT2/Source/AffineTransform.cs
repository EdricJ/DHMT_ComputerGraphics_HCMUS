...
namespace lab2
{
    ...
    class AffineTransform
    {
        ...
        private bool isSelectControlPoint(Point pQ) // dua vao diem select, xem co phai select control point hay k
        {
            foreach (Point p in controlPoints)
            {
                if (pQ.X >= (p.X - Config.DISTANCE_POINT) && pQ.X <= (p.X + Config.DISTANCE_POINT) && pQ.Y >= (p.Y - Config.DISTANCE_POINT) && pQ.Y <= (p.Y + Config.DISTANCE_POINT)) //xét control point là điểm nằm giữa đường biên 
                {
                    return true;
                }
            }
        ...
        public bool scale(Point nStart, Point nEnd)
        {
            ...
            for (int i = 0; i < controlPoints.Count; i++)
            {
                ...
                controlPoints[i] = new Point(x, y);     //tạo các điểm mới sau khi scale 
            }
            return true;
        }
    }
